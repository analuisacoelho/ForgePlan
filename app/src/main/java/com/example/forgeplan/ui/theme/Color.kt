package com.example.forgeplan.ui.theme

import androidx.compose.ui.graphics.Color

val ForgeDark = Color(0xFF0D0F2B)
val ForgeDarkSurface = Color(0xFF171A4A)
val ForgeDarkSurfaceElevated = Color(0xFF1F2260)

val ForgePrimary = Color(0xFF171A4A)
val ForgePurple = Color(0xFF2F2C79)

val ForgeGold = Color(0xFFE8C39E)
val ForgeGoldLight = Color(0xFFF5DDBB)

val ForgeSurface = Color(0xFFECEBFF)
val ForgeSurfaceSoft = Color(0xFFF4F2FF)
val ForgeSurfaceDark = Color(0xFFCBC9FF)

val ForgeBackground = Color(0xFFFAFAFA)
val ForgeCream = Color(0xFFF5E1CE)

val ForgeWhite = Color(0xFFFFFFFF)
val ForgeBlack = Color(0xFF111111)
val ForgeTextMuted = Color(0xFF5D5D6B)

val ForgeError = Color(0xFFB3261E)